...
#define previewFrameRetina (CGRect){ 0, ([UIScreen mainScreen].bounds.size.height - [UIScreen mainScreen].bounds.size.width*0.635)/2, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.width*0.635}
#define previewFrameRetina_4 (CGRect){ 0, ([UIScreen mainScreen].bounds.size.height - [UIScreen mainScreen].bounds.size.width*0.635)/2 , [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.width*0.635}
...
- (id) initWithFrame:(CGRect)frame captureSession:(AVCaptureSession *)captureSession
{
    ...
    if ( self ) {
        ...
        layer.string = @"将身份证填满整个拍摄窗口";
        layer.fontSize = 14;
        layer.contentsScale = 2;
        layer.font = (__bridge CFTypeRef)(@"HiraKakuProN-W3");
        layer.alignmentMode = @"center";
        layer.frame = CGRectMake(0, _previewLayer.bounds.size.height/2 - 5, _previewLayer.bounds.size.width, _previewLayer.bounds.size.height/2);
        
        [_previewLayer addSublayer:layer];
...
- (UIButton *) triggerButton
{
    if ( !_triggerButton ) {
        ...
        [_triggerButton setFrame:(CGRect){ CGRectGetMidX(self.bounds) - 35, [UIScreen mainScreen].bounds.size.height - 150, 70, 70 }];
